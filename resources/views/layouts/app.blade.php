...
<body class="bg-gray-900 flex flex-col md:flex-row" x-data="{ sidebarOpen: false }">

    <!-- Sidebar Desktop -->
<aside class="hidden md:flex flex-col justify-between w-64 bg-gray-800 h-screen shadow-md border-r border-gray-700 overflow-y-auto">
        <div>
            <!-- Header -->
...
<!-- Sidebar Mobile -->
<aside class="fixed inset-y-0 left-0 w-64 bg-gray-800 shadow-md z-50 overflow-y-auto transform md:hidden"
       x-show="sidebarOpen"
       x-transition:enter="transition ease-out duration-300"
    ...
    <nav class="p-4">
        <ul class="space-y-2 text-white">
            <li>
                <a href="{{ route('dashboard') }}"
                   class="block px-3 py-2 rounded hover:bg-gray-700 {{ request()->routeIs('dashboard') ? 'bg-blue-800 text-white' : '' }}">
                    Dashboard
                </a>
            </li>
            <li>
                <a href="{{ route('barang') }}"
                   class="block px-3 py-2 rounded hover:bg-gray-700 {{ request()->routeIs('barang') ? 'bg-blue-800 text-white' : '' }}">
                    Barang & Kategori
                </a>
            </li>
            <li>
                <a href="{{ route('barangmasuk') }}"
                   class="block px-3 py-2 rounded hover:bg-gray-700 {{ request()->routeIs('barangmasuk') ? 'bg-blue-800 text-white' : '' }}">
                    Barang Masuk
                </a>
            </li>
            <li>
                <a href="{{ route('barangkeluar') }}"
                   class="block px-3 py-2 rounded hover:bg-gray-700 {{ request()->routeIs('barangkeluar') ? 'bg-blue-800 text-white' : '' }}">
                    Barang Keluar
                </a>
            </li>
            <li>
                <a href="{{ route('laporan') }}"
                   class="block px-3 py-2 rounded hover:bg-gray-700 {{ request()->routeIs('laporan') ? 'bg-blue-800 text-white' : '' }}">
                    Laporan
                </a>
            </li>
            <li class="mt-8">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full text-left px-3 py-2 rounded hover:bg-red-600 text-red-500 font-bold">
                        Logout
                    </button>
                </form>
            </li>
    ...
    <!-- Content -->
<main class="flex-1 p-4 md:p-6 overflow-y-auto w-full min-h-screen">
        <!-- Navbar top untuk mobile -->
<div class="flex items-center justify-between md:hidden mb-4">
            <button @click="sidebarOpen = !sidebarOpen" class="text-gray-300 focus:outline-none z-50">
                <!-- Hamburger icon -->
            ...
            <h1 class="ml-4 text-lg font-bold text-white">Inventaris</h1>
        </div>
