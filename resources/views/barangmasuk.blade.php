...
<div class="fixed inset-0 bg-black bg-opacity-40 z-40 flex items-center justify-center p-4 sm:p-6">
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-3xl relative">
        ...
        <form action="{{ route('barangmasuk.store') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div class="flex justify-end gap-2">
                <a href="{{ route('barangmasuk') }}"
                    class="px-4 py-2 text-gray-600 border rounded hover:bg-gray-100">Batal</a>
                <button type="submit"
...
{{-- <div id="scannerModal"
    class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center hidden z-50 p-4 sm:p-6">
    <div class="bg-white p-4 rounded-lg w-full max-w-md relative">
        <video id="scanner" class="w-full"></video>
        <button type="button" onclick="stopScanner()"
            class="absolute top-2 right-2 bg-red-500 text-white px-2 py-1 rounded">X</button>
    </div>
</div> --}}
